...
import { Email, Location, LogoDiscord, LogoGithub, LogoLinkedin, PhoneFilled } from "@carbon/icons-react";
...
export default function Contact(): JSX.Element {
  return (
    <ContentWrapper element="section" className="my-24 py-9 md:bg-none md:py-0 scroll-mt-28" id="contact">
...
type TContactInfo = {
  icon: CarbonIconType;
  value: string;
  className?: TClassNames<"container" | "value">;
};

const ContactPersonalInfo: FC<TContactInfo> = (props) => {
  return (
    <li className={`flex flex-wrap items-center gap-4 rounded-lg px-6 py-4 ${props.className?.container}`}>
      <props.icon className="h-6 w-6 md:h-9 md:w-9 " />
      <span className={`font-medium text-slate-50 md:text-lg ${props.className?.value}`}>{props.value}</span>
    </li>
  );
};

const contactPersonalInfoOpts: TContactInfo[] = [
  { icon: Email, value: "gpmotta21@gmail.com" },
  { icon: PhoneFilled, value: "+55 2197426-4416" },
  {
    icon: Location,
    value: "Rio de Janeiro, Brazil",
    className: { container: "bg-sulu-300 text-black", value: "!text-black" },
  },
];

const contactSocialMediaOpts: TContactInfo[] = [
  {
    icon: LogoGithub,
    value: "https://github.com/gpmotta21",
  },
  {
    icon: LogoLinkedin,
    value: "https://www.linkedin.com/in/gpmotta21/",
  },
  {
    icon: LogoDiscord,
    value: "",
